...
class BurgerIllustration extends StatelessWidget {
  ...
  const BurgerIllustration({super.key, this.size = 200, this.backgroundColor = const Color(0xFFF4E0CC)});
...
class _StaticBurgerPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    void drawLayer(double dy, double w, double h, Color color, {double radius = 100}) {
      final paint = Paint()..color = color;
      final rect = Rect.fromCenter(center: Offset(cx, baseY - dy), width: w, height: h);
    ...
    canvas.drawOval(
      Rect.fromCenter(center: Offset(cx, baseY + size.height * 0.16), width: size.width * 0.6, height: size.height * 0.06),
      shadowPaint,
    );

    drawLayer(0, size.width * 0.5, size.height * 0.13, const Color(0xFFC97A32), radius: 14);
    drawLayer(size.height * 0.1, size.width * 0.46, size.height * 0.1, const Color(0xFF6B4A2F));
    drawLayer(size.height * 0.17, size.width * 0.49, size.height * 0.06, const Color(0xFFF2B705));
    drawLayer(size.height * 0.22, size.width * 0.52, size.height * 0.07, const Color(0xFF7EA32A));

    final domePaint = Paint()..color = const Color(0xFFE79A3F);
    final domeRect = Rect.fromCenter(center: Offset(cx, baseY - size.height * 0.33), width: size.width * 0.54, height: size.height * 0.23);
    canvas.drawArc(domeRect, 3.14, 3.14, false, domePaint..style = PaintingStyle.fill);

    final sesamePaint = Paint()..color = Colors.white;
    for (final dx in [-0.14, -0.05, 0.06, 0.15]) {
      canvas.drawCircle(Offset(cx + dx * size.width, baseY - size.height * 0.38), size.width * 0.012, sesamePaint);
    }
  }
...
class IngredientTile extends StatelessWidget {
  ...
  const IngredientTile({super.key, required this.icon, required this.color, this.size = 110});
