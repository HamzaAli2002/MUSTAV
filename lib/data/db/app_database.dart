...
class AppDatabase {
  ...
  Future<DateTime?> menuCachedAt() async {
    final db = await database;
    final rows = await db.query('menu_items', columns: ['cachedAt'], orderBy: 'cachedAt DESC', limit: 1);
  ...
  Future<void> cacheLocations(List<StoreLocation> stores) async {
    ...
    for (final s in stores) {
      batch.insert('locations', s.toMap(), conflictAlgorithm: ConflictAlgorithm.replace);
    }
    await batch.commit(noResult: true);
  }
  ...
  Future<void> upsertCartItem(CartItem item) async {
    ...
    await db.transaction((txn) async {
      await txn.insert('cart_items', item.toMap(), conflictAlgorithm: ConflictAlgorithm.replace);
      await txn.delete('cart_item_addons', where: 'cartItemId = ?', whereArgs: [item.cartItemId]);
      for (final addOn in item.addOns) {
        await txn.insert('cart_item_addons', {'cartItemId': item.cartItemId, 'addOnId': addOn.id});
      }
    });
  }
  ...
  Future<void> removeCartItem(String cartItemId) async {
    final db = await database;
    await db.transaction((txn) async {
      await txn.delete('cart_items', where: 'cartItemId = ?', whereArgs: [cartItemId]);
      await txn.delete('cart_item_addons', where: 'cartItemId = ?', whereArgs: [cartItemId]);
    });
  }
  ...
  Future<List<CartItem>> readCart(List<Burger> menuById) async {
    ...
    for (final row in rows) {
      ...
      if (burger == null) continue; // burger no longer in menu cache — skip defensively
      final addOnRows = await db.query(
        'cart_item_addons',
        where: 'cartItemId = ?',
        whereArgs: [row['cartItemId']],
      );
      final addOns = addOnRows.map((r) => AddOnCatalog.byId(r['addOnId'] as String)).toList();
  ...
  Future<void> saveOrder(Order order) async {
    ...
    await db.transaction((txn) async {
      ...
      for (final item in order.items) {
        ...
        for (final addOn in item.addOns) {
          await txn.insert('order_item_addons', {'orderItemId': orderItemId, 'addOnId': addOn.id});
        }
      }
    });
  }
  ...
  Future<void> updateOrderStatus(String orderId, OrderStatus status) async {
    final db = await database;
    await db.update('orders', {'status': status.name}, where: 'orderId = ?', whereArgs: [orderId]);
  }

  Future<Order?> readLatestOrder(List<Burger> menuById, List<StoreLocation> stores) async {
    final db = await database;
    final orderRows = await db.query('orders', orderBy: 'placedAt DESC', limit: 1);
    if (orderRows.isEmpty) return null;
    final orderRow = orderRows.first;
    final orderId = orderRow['orderId'] as String;
    final itemRows = await db.query('order_items', where: 'orderId = ?', whereArgs: [orderId]);
    ...
    return Order(
      ...
      placedAt: DateTime.fromMillisecondsSinceEpoch(orderRow['placedAt'] as int),
